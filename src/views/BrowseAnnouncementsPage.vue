...
// Extract source code from ID
const getSourceCode = (id: string): string => {
  const match = id.match(/(?:group|announcement)\/([^/]+)/)
  return match ? match[1] : 'unknown'
}

const getAnnouncementRef = (id: string): string => {
  return id.replace('https://www.ammitto.org/announcement/', '')
}
...
// Groups are used as proxy for announcements since they reference announcement IDs
const announcements = computed<Announcement[]>(() => {
  return groups.value.map((group: SanctionGroup) => ({
    id: group.announcement_id || group.id,
    group_id: group.id,
    effective_date: group.effective_date,
    entity_count: group.entity_count,
    ref: getAnnouncementRef(group.announcement_id || group.id),
    source: getSourceCode(group.announcement_id || group.id),
    title: group.announcement_title
  }))
})

// Compute source counts from announcements
const sourceCounts = computed(() => {
  const counts: Map<string, number> = new Map()
  for (const announcement of announcements.value) {
    counts.set(announcement.source, (counts.get(announcement.source) || 0) + 1)
  }
  return Array.from(counts.entries()).map(([code, count]) => ({ code, count }))
})

// Filter announcements by selected source
const filteredAnnouncements = computed(() => {
  if (!selectedSource.value) return announcements.value
  ...
  try {
    ...
    const fetched = await mapWithPool(
      ...
      async (node) => {
        ...
        return fetchNodeOnce<SanctionGroup>(
    ...
    // holds no announcements.
    const outcome = hydrationOutcome(fetched, nodes.length)
    if (outcome.allFailed) {
      error.value = 'Could not load announcements. Check your connection and reload.'
      return
    }
  ...
  <div class="min-h-screen">
    <div class="container-wide py-8">
      ...
      <div class="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p class="text-light-muted dark:text-dark-muted">
            Official announcements from sanctioning authorities.
          </p>
        </div>
        <div class="text-right shrink-0">
          <div class="text-2xl font-bold text-light-fg dark:text-dark-fg">{{ filteredAnnouncements.length }}</div>
      ...
      <div v-else-if="filteredAnnouncements.length === 0" class="text-center py-12">
        <p class="text-light-muted dark:text-dark-muted">No announcements found for the selected filter.</p>
      </div>
      ...
      <div v-else class="space-y-4">
        <RouterLink
          ...
          :key="announcement.id"
          :to="`/announcement/${announcement.ref}`"
          ...
          <div class="flex items-start justify-between gap-4">
            <div class="flex-1">
              <h3 class="text-lg font-semibold text-light-fg dark:text-dark-fg mb-1">
                {{ announcement.title || `Announcement ${announcement.ref}` }}
              </h3>
              <p v-if="announcement.title" class="text-xs text-light-muted dark:text-dark-muted mb-2 font-mono">
                {{ announcement.ref }}
              </p>
              <div class="flex flex-wrap gap-2">
                <Badge v-if="announcement.effective_date" variant="default">
                  {{ formatDate(announcement.effective_date) }}
                </Badge>
                <Badge v-if="announcement.entity_count" variant="default">
                  {{ announcement.entity_count }} entities
                </Badge>
              </div>
            </div>
