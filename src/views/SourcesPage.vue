    ...
    <div class="container-wide py-12">
      ...
      <div class="mt-12 p-6 glass-card">
        ...
        <p class="text-light-muted dark:text-dark-muted">
          <!--
            The rebuild republishes data already collected on each source's own
            schedule, so the date is the build time of the copy and one source
            can be older. Wording such as "from the official sources" would imply
            the rebuild contacts each authority; freshnessClaimWiring.test.js
            pins that. The build sentence is gated on the date so the prerendered
            page, which has no stats yet, never shows a sentence with a blank in it.
          -->
          <template v-if="asOf">
            This site was last built
            <span class="font-medium text-light-text dark:text-dark-text">{{ asOf }}</span>.
          </template>
          An individual source can be older than the site's build date, since each is collected from its own authority separately.
        </p>
