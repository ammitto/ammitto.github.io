...
test('sourceCodeFromRef agrees with the other helpers on the same ref', () => {
  ...
  //
  // getEntityApiUrl was removed from entityUrls.ts (superseded by
  // getEntityNodePath, which takes `base` explicitly rather than
  // hardcoding it) after this test was first written; the cross-check
  // now goes through the replacement instead of the removed function.
  ...
  assert.equal(getEntityNodePath(ref, '/'), '/api/v1/node/entity/uk/aqd0087.jsonld')
