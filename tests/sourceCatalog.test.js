...
// Sorted so the example this file picks does not depend on the manifest
// generator's key order; the contract holds for any of them, but a stable
// choice keeps a failure readable.
const NOT_SERVED = Object.keys(manifest.pending_sources).sort()
...
test('publishing a held-back source needs only its removal from the set', () => {
  ...
  // Needs a held-back source to release. When there are none the contract
  // is still true and there is simply nothing to demonstrate it with, which
  // is a pass rather than a failure — and it is a state this repo is
  // actively working towards, since ru is the last entry.
  if (NOT_SERVED.length === 0) return
